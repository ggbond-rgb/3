...
static int sisa_index(double value)
{
	...
	else if (value<=0.49) return (int)(value/0.01+0.5);
	else if (value<=0.98) return (int)((value-0.5)/0.02+0.5)+50;
	else if (value<=1.96) return (int)((value-1.0)/0.04+0.5)+75;
    return (int)((value-2.0)/0.16+0.5)+100;
}
...
static void decode_obsh(FILE *fp, char *buff, double ver, int *tsys,
                        char tobs[][MAXOBSTYPE][4], nav_t *nav, sta_t *sta, int *type_change_flag)
{
    if (type_change_flag != NULL) {
        *type_change_flag = 0;
    }
    ...
    *ver=2.10; *type=' '; *sys=SYS_GPS; *tsys=TSYS_GPS;
    ...
    while (fgets(buff,MAXRNXLEN,fp)) {
        ...
        switch (*type) { /* file type */
            case 'O': decode_obsh(fp,buff,*ver,tsys,tobs,nav,sta,NULL); break;
...
static int addobsdata(obs_t *obs, const obsd_t *data)
{
    ...
    gtime_t temp_a = epoch2time(temp);

    if(data->time.time == temp_a.time && data->time.sec==temp_a.sec){
        // char buff_temp[64] = time_str(obs[0].time,3);
        int a = 0;
    }
                       ...
                       sta_t *sta)
{
    ...
    while (fgets(buff,MAXRNXLEN,fp)) {
        ...
        if (i==0) {
            ...
            /* decode RINEX observation data file header */
            decode_obsh(fp,buff,ver,tsys,tobs,NULL,sta,&obs_type_change_flag);
            if(obs_type_change_flag) ++i;
        }
        if (++i>nsat) return n;
    }
    return -1;
...
/* decode ephemeris ----------------------------------------------------------*/
// Rinex4 GPS LNAV same to Rinex3
// Rinex4 BDS CNAV1 add 
static int decode_eph(double ver, int sat, gtime_t toc, const double *data,
                      eph_t *eph,E_EphType type)
{
    eph_t eph0={0};
    int sys;
    char sat_id1[8];
    ...
    *eph=eph0;
    ...
    if (sys==SYS_GPS||sys==SYS_QZS) {
    ...
    else if (sys==SYS_CMP&type==Eph_EPH) { /* RINEX3 BeiDou D1D2 NAV */
        PPP_Glo.BDS_CNV1_flag = 0;
        eph->toc=bdt2gpst(eph->toc);  /* bdt -> gpst */
        eph->iode=(int)data[ 3];      /* AODE */
        eph->iodc=(int)data[28];      /* AODC */
        eph->toes=     data[11];      /* Toe (s) in BDT week */
    ...
    else if (sys==SYS_CMP&type==Eph_CEPH) { /* BeiDou Rinex4 CNAV1 */
        PPP_Glo.BDS_CNV1_flag = 1;
        /* BeiDou CNAV needs to read ADOT. The difference between RINEX3 and RINEX4 is the difference in A, one is deltaA and the other is sqrt A */
        eph->Adot=     data[3];
        eph->ndot=     data[20];

        satno2id(sat,sat_id1);

        int week = 0;
        time2bdt(eph->toc,&week);
        eph->toc=bdt2gpst(eph->toc);  /* bdt -> gpst */
        eph->iode=(int)data[38];      /* IODE */
        eph->iodc=(int)data[34];      /* IODC */
        eph->toes=     data[11];      /* Toe (s) in BDT week */

        eph->week=     week;      /* According to Rinex4, this parameter is no longer included, and users need to calculate the week using epoch(toc) */
        eph->toe=bdt2gpst(bdt2time(eph->week,data[11])); /* BDT -> GPST */
        ...
        /* According to "IGMAS_CNAV_RINEX3.0": CNAV1 carries the space signal accuracy index and space signal monitoring accuracy index SISA */
        eph->sisa[0] = data[23];
        eph->sisa[1] = data[24];
        eph->sisa[2] = data[25];
        eph->sisa[3] = data[26];
                       ...
                       int *type, eph_t *eph, geph_t *geph, seph_t *seph)
{
    ...
    /* set system mask */
    mask=set_sysmask(opt);
    E_NavRecType recType = E_NavRec_NONE;
    E_NavMsgType msgType = E_NavMsg_NONE;
    E_EphType ephType = E_Eph_NONE;

    while (fgets(buff,MAXRNXLEN,fp)) {

        if (i==0) {
            // This part refers to the readRnxNavB function in ginan
            // decode message type field
            if(ver>=4.0&&buff[0] == '>') {
                char typeStr[5] = "";
                sp = 4;
                strncpy(typeStr,buff+2,3);
                recType	= StringToNavRec(typeStr);
                ...
                msgType	= StringToNavMsg(typeStr);
                continue;
            ...
            if (recType == STO) {
                ...
                // Reserve interface for STO
                char code[19] = "";
                strncpy(code,buff+24,18);
                // Here should be three data[i++]=...
                i++;
                // TODO
            ...
            if (recType == E_NavRec_NONE)	recType = EPH;
            if (msgType == E_NavMsg_NONE)   msgType = defNavMsgType[sys];
            ...
            if(recType == EPH) {
                switch (msgType)
                {
                    ...
                    //
                    //     }
                    case CNV1://b2B用到的北斗CNAV星历
                        ephType=Eph_CEPH;
                        if (ver >= 4 & sys == SYS_CMP & i >= 39) {
                            if (!(mask & sys)) return 0;
                            *type = 0;
                             if(PPP_Glo.b2b_flag==1) {
                                 return decode_eph(ver, sat, toc, data, eph,ephType);
                             }
                             else
                                 return 0;
                            // return decode_eph(ver, sat, toc, data, eph,ephType);
...
extern int rnxcomment(rnxopt_t *opt, const char *format, ...) {
	char buff[256];
	int req = 0, i = 0, j = 0, indent = 0, n = 0, rem = 0;
    va_list ap;
    va_start(ap, format);
    req = vsnprintf(buff, sizeof(buff), format, ap);
    va_end(ap);
    if (req < 0) {
    ...
    for (j = 0, rem = strlen(buff); rem > 0; i++) {
        if (i >= MAXCOMMENT) return 0;
        indent = j > 0 ? 2 : 0; // Indent overflow lines
		n = rem > 60 - indent ? 60 - indent : rem;
