...
static char codepris[7][MAXFREQ][16]={  /* code priority for each freq-index */
    ...
    /* L1/E1/B1 L2/E5a/B3 L5/E5a/B2a E6/LEX/B3 E5(a+b)         */
...
static int code2freq_BDS(uint8_t code, double *freq)
{
    ...
    switch (obs[0]) {
		...
		case '7': *freq=FREQ2_CMP; return 2; /* B2b */
...
/* zero matrix -----------------------------------------------------------------
* generate new (int)zero matrix 
* args   : int    n,m       I   number of rows and columns of matrix
* return : matrix pointer (if n<=0 or m<=0, return NULL)
*-----------------------------------------------------------------------------*/
extern int *i_zeros(int n, int m)
{
    int *p;
...
/* multiply matrix -----------------------------------------------------------*/
extern void matmul1(const char *tr, int n, int k, int m,
	const double *A, const double *B, double *C)
{
...
extern void matmulp1(const char *tr, int n, int k, int m,
	const double *A, const double *B, double *C)
{
	int f = (tr[0] != 'N') * 2 + (tr[1] != 'N');
...
extern void matmulm1(const char *tr, int n, int k, int m,
	const double *A, const double *B, double *C)
{
	int f = (tr[0] != 'N') * 2 + (tr[1] != 'N');
...
/* inverse of matrix ---------------------------------------------------------*/
extern int matinv1(double *A, int n)
{
	double d, *B;
	int i, j, *indx;
...
/* solve linear equation -----------------------------------------------------*/
extern int solve1(const char *tr, const double *A, const double *Y, int n,
	int m, double *X)
{
	double *B = mat(n, n);
	int info;
...
static int readantex(const char *file, pcvs_t *pcvs)
{
	...
	char buff[1024],csys;   //buff[256] -> buff[1024]
	...
	while (fgets(buff,sizeof(buff),fp)) {
		...
		if (strstr(buff+60,"START OF ANTENNA")) {
			...
			pcv.var = (double (*)[1600])malloc(sizeof(double)*NSYS_USED*NFREQ*1600);
		}
...
//				freq = f+2*NFREQ;
				if (f==2) freq = 1+2*NFREQ;
                else if (f==6) freq = 2+2*NFREQ;
				else if (f==7) freq = 3+2*NFREQ;
				else freq=0;
			}
			else if (csys=='E') {
				if (f==1) freq = f+3*NFREQ;
				else if (f==5) freq = 2+3*NFREQ;
				else if (f==7) freq = 3+3*NFREQ;
				else freq=0;
			}
			else if (csys=='J') {
...
extern int readblq(const char *file, const char *sta, double *odisp)
{
    ...
    /* Extract first 4 characters of station name and convert to uppercase */
    if (sscanf(sta, "%4s", staname) < 1) return 0;  // Modification 1: Capture first 4 chars
    ...
    while (fgets(buff, sizeof(buff), fp)) {
        ...
        /* Extract first 4 characters from file line and convert to uppercase */
        if (sscanf(buff + 2, "%4s", name) < 1) continue;  // Modification 2: Capture first 4 chars
        for (p = name; (*p = (char)toupper((int)(*p))); p++);
        
        /* Compare only first 4 characters */
        if (strncmp(name, staname, 4)) continue;  // Modification 3: Use strncmp for 4-char comparison

        /* Read matched BLQ record */
...
static double interpvar0(int sat, double ang, const double *var)
{
	int i,sys,limit=18;
	double a;
	a=ang/5.0; /* ang=0-90 */
                     ...
                     int opt, double *dant)
{
	...
	if(strlen(pcv->type)==0) {
		for (i=0;i<NFREQ;i++) {
			...
			dant[i]=-dot(off,e,3)+(opt?interpvar0(0,90.0-azel[1]*R2D,pcv->var[ii]):0.0);
		}
		return ;
	}
	...
	for (i=0;i<NFREQ;i++) {
		...
		if (pcv->dazi!=0.0) 
			...
			dant[i]=-dot(off,e,3)+interpvar0(0,90.0-azel[1]*R2D,pcv->var[ii]);
	}
}
...
extern void antmodel_s(int sat, const pcv_t *pcv, double nadir, double *dant)
{
	...
	for (i=0;i<NFREQ;i++) {
		if (sys==SYS_GPS) {
			dant[i]=interpvar0(sat,nadir*R2D*5.0,pcv->var[i]);
			if (i==2) {
				dant[i]=interpvar0(sat,nadir*R2D*5.0,pcv->var[1]);
			}
		}
		else if (sys==SYS_GLO) {
			dant[i]=interpvar0(sat,nadir*R2D*5.0,pcv->var[i+NFREQ]);
			if (i==2) {
				dant[i]=interpvar0(sat,nadir*R2D*5.0,pcv->var[1+NFREQ]);
			}
		}
		else if (sys==SYS_CMP) {
			dant[i]=interpvar0(sat,nadir*R2D*5.0,pcv->var[i+2*NFREQ]);
			if (i==2) {
				dant[i]=interpvar0(sat,nadir*R2D*5.0,pcv->var[1+2*NFREQ]);
			}
		}
		else if (sys==SYS_GAL) {
			dant[i]=interpvar0(sat,nadir*R2D*5.0,pcv->var[i+3*NFREQ]);
			if (i==2) {
				dant[i]=interpvar0(sat,nadir*R2D*5.0,pcv->var[1+3*NFREQ]);
			}
		}
		else if (sys==SYS_QZS) {
			dant[i]=interpvar0(sat,nadir*R2D*5.0,pcv->var[i+4*NFREQ]);
			if (i==2) {
				dant[i]=interpvar0(sat,nadir*R2D*5.0,pcv->var[1+4*NFREQ]);
			}
		}
	}
...
/**
 * For my conf file
 * Trim whitespace characters from the start and end of the string.
 */
extern void trim(char *str) {
    char *start = str;  // Start of the string to trim
    ...
    // If the entire string was whitespace, null terminate it
    if (*start == '\0') {
        *str = '\0';
        return;
    }
